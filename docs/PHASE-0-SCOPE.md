...
3. 📤 **Archive's "Export CSV" exports no text.** It reads `entry.title` and `entry.content`, and nothing in the app ever writes either — Quick Capture writes `notes`, reflections write `answers`. Every row is date, type, `""`, `""`. `src/services/searchService.ts:355-384`, `Archive.tsx:292`. Small fix; phase 0 pins it.
